...
export default function FavoriteComponentsScreen() {
  const { user } = useAuth();
  ...
  const { result, loading, error } = useFavoriteComponents({
      ...
      <div className={styles.inner}>
        <div className={styles.header}>
          <TypeDropdown
            ...
            pcComponentTypes={COMPONENT_TYPES}
          />
          <SearchBar
            value={searchInput}
            onChange={setSearchInput}
            placeholder={`Search favourite ${currentType.label}s...`}
          />
        </div>
          ...
          <div className={styles.content}>
            ...
            {error ? (
              <div className={styles.errorState}>
                <p className={styles.errorText}>Could not load favourites: {error}</p>
              </div>
            ) : (
              <>
                ...
                {!loading && result?.total === 0 && (
                  <div className={styles.emptyState}>
                    <p>No favourite {currentType.label}s saved yet.</p>
                  </div>
                )}
              </>
            )}
